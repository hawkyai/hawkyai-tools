  ...
  try {
    ...
    const leadData = {
      data: [
        {
          id: '1',
          Company: '', // Not present in this form, so leave blank
          Last_Name: formData.name || '',
          First_Name: '', // Not present in this form, so leave blank
          Phone: formData.phone || '',
          Email: formData.email || '',
          Lead_Source: formData.hear_about || 'Website / Get Demo'
        }
      ],
      trigger: [
        'workflow'
...
export async function POST(request: Request) {
  try {
    ...
    console.log("Received data in /api/get-data (POST):", data);
    ...
    // Store in Zoho
    const zohoResult = await createZohoLead(data, submissionId);
    ...
    const slackResult = await sendSlackNotification(data, submissionId);
    ...
    console.error("Error in get-data API (POST):", error);
    const errorMessage = error instanceof Error ? error.message : String(error)
    return NextResponse.json({ 
      success: false, 
      error: errorMessage || "Failed to process data",
